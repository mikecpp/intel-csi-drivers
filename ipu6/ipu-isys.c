...
#if IS_ENABLED(CONFIG_VIDEO_INTEL_IPU_USE_PLATFORMDATA)
static int isys_register_ext_subdev(struct ipu_isys *isys, struct ipu_isys_subdev_info *sd_info)
{
...
static int isys_probe(struct ipu_bus_device *adev)
{
	...
	ipu_trace_init(adev->isp, isys->pdata->base, &adev->dev, isys_trace_blocks);
